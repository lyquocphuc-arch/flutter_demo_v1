...
class _HomeScreenState extends State<HomeScreen> {
  ...
  String _calculateStatus(Room room) {
    ...
    for (var b in roomBookings) {
      ...
      if (start.isBefore(b.checkOut) && end.isAfter(b.checkIn)) {
        return b.status == BookingStatus.CheckedIn ? "OCCUPIED" : "RESERVED";
      }
    }
    return "AVAILABLE";
  }
  ...
  int _countActiveBookings(Room room) {
    return _bookings.where((b) => b.roomId == room.id && (b.status == BookingStatus.Confirmed || b.status == BookingStatus.CheckedIn)).length;
  }

  Future<void> _pickDateTime() async {
    final date = await showDatePicker(context: context, initialDate: _viewTime, firstDate: DateTime.now(), lastDate: DateTime.now().add(const Duration(days: 30)));
    if (date == null) return;
    if(!mounted) return;
  ...
  Widget build(BuildContext context) {
    List<Room> filteredRooms = _allRooms.where((room) {
      if (!(_selectedRoomTypes[room.type] ?? true) || !(_selectedBedTypes[room.bedType] ?? true)) return false;
      if (_showAvailableOnly && _calculateStatus(room) != 'AVAILABLE') return false;
      return true;
    }).toList();
    ...
    return Scaffold(
      ...
      drawer: Drawer(
        child: ListView(children: [
          ...
          ListTile(leading: const Icon(Icons.list_alt), title: const Text("Quản lý Đặt phòng"), onTap: () {
            Navigator.pop(context);
            Navigator.push(context, MaterialPageRoute(builder: (_) => const BookingHistoryScreen()));
          }),
          ListTile(leading: const Icon(Icons.settings), title: const Text("Quản lý Phòng"), onTap: () {
            Navigator.pop(context);
            Navigator.push(context, MaterialPageRoute(builder: (_) => const RoomManagerScreen()));
          }),
          const Divider(),
          ListTile(leading: const Icon(Icons.logout), title: const Text("Đăng xuất"), onTap: () => Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const LoginScreen()))),
        ]),
      ),
      body: Column(
        children: [
          Container(
            ...
            child: Row(children: [
              Expanded(
                child: InkWell(onTap: _pickDateTime, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              DropdownButton<int>(
                value: _durationHours, underline: Container(),
          ...
          ExpansionTile(
            ...
            children: [
              SwitchListTile(title: const Text("Chỉ hiện phòng Trống"), value: _showAvailableOnly, activeColor: Colors.green, onChanged: (val) => setState(() => _showAvailableOnly = val)),
              const Divider(height: 1),
              const Padding(padding: EdgeInsets.all(16), child: Align(alignment: Alignment.centerLeft, child: Text("Loại phòng:", style: TextStyle(fontWeight: FontWeight.bold)))),
              Wrap(spacing: 10, children: _selectedRoomTypes.keys.map((key) => Row(mainAxisSize: MainAxisSize.min, children: [
                Checkbox(value: _selectedRoomTypes[key], onChanged: (val) => setState(() => _selectedRoomTypes[key] = val!)),
                Text(key)
              ])).toList()),
              const Divider(height: 1),
              const Padding(padding: EdgeInsets.all(16), child: Align(alignment: Alignment.centerLeft, child: Text("Loại giường:", style: TextStyle(fontWeight: FontWeight.bold)))),
              Wrap(spacing: 10, children: _selectedBedTypes.keys.map((key) => Row(mainAxisSize: MainAxisSize.min, children: [
                Checkbox(value: _selectedBedTypes[key], onChanged: (val) => setState(() => _selectedBedTypes[key] = val!)),
                Text(key)
              ])).toList()),
            ],
          ),
                    ...
                    child: Card(
                      ...
                      child: Column(children: [
                        Expanded(flex: 3, child: Stack(fit: StackFit.expand, children: [
                          Image.network(room.image, fit: BoxFit.cover, errorBuilder: (c, e, s) => Container(color: Colors.grey)),
                          if (bookingCount > 0) Positioned(top: 5, right: 5, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: Colors.blueAccent, borderRadius: BorderRadius.circular(4)), child: Text("$bookingCount đơn", style: const TextStyle(color: Colors.white, fontSize: 10)))),
                        ])),
                        Expanded(flex: 2, child: Container(padding: const EdgeInsets.all(8), color: Colors.white, child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
